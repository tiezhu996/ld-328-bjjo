...
export default function Profile() {
  ...
  useEffect(() => {
    if (user) form.setFieldsValue({ name: user.name, phone: user.phone, avatar: user.avatar });
  }, [user, form]);

  async function onFinish(values: { name: string; avatar?: string }) {
    setSaving(true);
    try {
      const updated = await save({ name: values.name, avatar: values.avatar || '' });
      updateUser(updated);
  ...
  return (
    <Card title="个人中心" style={{ maxWidth: 560 }}>
      <Form form={form} layout="vertical" onFinish={onFinish}>
        ...
        <Form.Item name="name" label="昵称" rules={[{ required: true, max: 50 }]}><Input /></Form.Item>
        <Form.Item name="avatar" label="头像地址">
          <Upload listType="picture" maxCount={1} beforeUpload={() => false} onChange={(info) => {
            ...
            <Button icon={<UploadOutlined />}>上传头像（base64 预览）</Button>
          </Upload>
        </Form.Item>
